      ...
      <div className="topicContainer w-full flex justify-center bg-stone-200 opacity-80">
            ...
            >
              {designs.map(design => (
                ...
                <Card key={design.id} className="max-w-100 flex-col-reverse mb-10 bg-stone-100">
                  <CardContent>
                  <p className="font-courier optionTitle text-left text-md">{design.title}</p> {/* ← Artist info available! */}              

                    <img src={design.imageUrl[0]} alt={design.title} className="w-full h-50"/>
                  </CardContent>
                  <CardHeader>
                  <CardTitle className="font-courier text-gray-500 text-left text-sm">{design.type}</CardTitle>
                  </CardHeader>
                </Card>
                </motion.div>
              ))}
